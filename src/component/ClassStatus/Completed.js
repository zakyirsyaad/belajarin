...
export default function Completed({ match }) {
    const { params: { uid, status } } = match;
    const [data, setData] = useState([]);

    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`https://belajarin-tau.vercel.app/member/class/${uid}/${status}`);
